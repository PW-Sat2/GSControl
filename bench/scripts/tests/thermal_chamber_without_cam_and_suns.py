...
@make_test
def climatic_chamber_calibration_without_cam_and_suns(duration, filename_base, suns_gain, suns_itime):
    bar = progressbar.ProgressBar(max_value=duration.total_seconds()/60)

    radfet = RadFET_Logger(filename_base)
    suns = SunS_Logger(filename_base)
    ...
    PrintLog("enable iMTQ LCL", obc.enable_lcl(7))
