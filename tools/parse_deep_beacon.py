...
from emulator.beacon_parser.deep_sleep_beacon_parser import DeepSleepBeaconParser
...
class ParseDeepBeacon:
    @staticmethod
    def parse(frame):
        if isinstance(frame, DeepSleepBeacon):
            ...
            parsers = DeepSleepBeaconParser().GetParsers(reader, store)
            parsers.reverse()
    ...
    def convert(beacon):
        for k, v in beacon.items():
            for k2, v2 in beacon[k].items():
                beacon[k][k2] = ParseDeepBeacon.convert_values(v2)

        return beacon

    @staticmethod
    def convert_json(beacon):
        return json.dumps(beacon, default=ParseDeepBeacon.convert_values, sort_keys=True, indent=4)
